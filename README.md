...
- personal - пользовательская папка?
- .projectile - для жизни хватает встроенного project - удалить
- sample - ??? # не понятно про какие примеры идёт речь - там лежит файл подключения модулей (обвязок вокруг python...).
- utils - ???
- vendor - ???
...
modules - фишка prelude. Благодаря модулям выстраивается окружение для python,
к примеру, и позволяет отдельно от всего остального выполнять настройку
множества пакетов для работы. Так же ключевым является функция
prelude-require-package которая используется в модуле и через которую
указывается какие расширения должны стоять.
